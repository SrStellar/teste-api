...
export async function deleteAppointment(req: AuthRequest, res: Response, next: Function) { try { const id = req.params.id; await cancelAppointment(id, req.user!.id); res.status(204).send(); } catch (e) { next(e); } }